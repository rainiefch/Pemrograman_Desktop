...
Public Class Form1
    ...
    Private Sub CopyPathToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles CopyPathToolStripMenuItem1.Click
        If lvFile.SelectedItems.Count = 1 Then
    ...
    Private Sub ExportToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles ExportFileToolStripMenuItem.Click
    ...
    Private Sub HapusFileTerpilih()
        If lvFile.SelectedItems.Count > 0 Then
            Dim filePath As String = lvFile.SelectedItems(0).SubItems(1).Text
            If MessageBox.Show("Do you want to delete this file?", "Yes", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                Try
                    System.IO.File.Delete(filePath)
                    lvFile.SelectedItems(0).Remove()
                Catch ex As Exception
                    MessageBox.Show("File deleting error: " & ex.Message)
                End Try
            End If
        End If
    End Sub

    Private Sub GantiNamaFile()
        If lvFile.SelectedItems.Count > 0 Then
            Dim filePath As String = lvFile.SelectedItems(0).SubItems(1).Text
            Dim newName As String = InputBox("Input new name:", "Rename", System.IO.Path.GetFileName(filePath))
            If Not String.IsNullOrEmpty(newName) Then
                Try
                    Dim newFilePath As String = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(filePath), newName)
                    System.IO.File.Move(filePath, newFilePath)
                    lvFile.SelectedItems(0).SubItems(1).Text = newFilePath
                Catch ex As Exception

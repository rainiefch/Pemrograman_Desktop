...
Public Class Form1
    ...
    Private Sub lvImage_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles lvImage.SelectedIndexChanged
        ...
        If lvImage.SelectedItems.Count > 0 Then
            Dim selectedFilePath As String = lvImage.SelectedItems(0).Tag.ToString

            If File.Exists(selectedFilePath) Then
                Try
                    ...
                    Using fs As New FileStream(selectedFilePath, FileMode.Open, FileAccess.Read)
                        Using ms As New MemoryStream()
                            ...
                            Using img As Image = Image.FromStream(ms)
                                ...
                                ToolStripStatusReady.Text = selectedFilePath
                    ...
                    End Using
                Catch ex As Exception
                    MessageBox.Show("Error loading image or reading basic info: " & ex.Message & " (Path: " & selectedFilePath & ")", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Console.WriteLine(ex.ToString())
                End Try
            Else
                MessageBox.Show("File not found: " & selectedFilePath, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
        End If
